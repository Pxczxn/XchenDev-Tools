...
runtime = '''import type { LaunchProfile, LaunchSessionInfo } from "../ipc/types";
...
'''
Path("src/lib/projectRuntime.ts").write_text(runtime, encoding="utf-8", newline="\n")

runtime_test = '''import { describe, expect, it } from "vitest";
...
'''
Path("src/lib/projectRuntime.test.ts").write_text(
    runtime_test, encoding="utf-8", newline="\n"
)
...
page = Path("src/pages/ProjectsPage.tsx")
text = page.read_text(encoding="utf-8")
start_marker = "  async function onStart(profile: LaunchProfile) {"
end_marker = "\n  return (\n"
start = text.find(start_marker)
end = text.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit("ProjectsPage start/stop block markers not found")

replacement = '''  function rememberStartedSession(
...
'''
page.write_text(text[:start] + replacement + text[end:], encoding="utf-8", newline="\n")
...
if css.count(marker) != 1:
    raise SystemExit("app.css list-card-actions marker mismatch")
css_path.write_text(css.replace(marker, addition, 1), encoding="utf-8", newline="\n")
